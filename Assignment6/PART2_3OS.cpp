...
int main(int argc, char* argv[]) {
    ...
    if (pid == -1) {
        ...
        for (int i = 0; i < height; ++i) {
            write(pipefd[1], &intermediate_pixels[i][0], sizeof(Pixel) * width); // Write each row to the pipe
            cout<<"Hello"<<endl;
        }
        cout<<"Completed Horizontal Blur "<<endl;
        cout<<"1";
        ...
        vector<vector<Pixel>> updated_pixels(height, vector<Pixel>(width));
        cout<<"2";
        wait(NULL);
        for (int i = 0; i < height; ++i) {
            read(pipefd[0], &updated_pixels[i][0], sizeof(Pixel) * width); // Read each row from the pipe
        }
        close(pipefd[0]); // Close the read end of the pipe
        convert_to_grayscale(updated_pixels, original_pixels);
