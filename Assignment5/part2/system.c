			...
			int cpu)
{
	...
	printf("-----Time quanta allotted = %d\n", p->p_quantum_size_ms); 
	// (current process quantum) - (cpu time left)
	// cpu_time_2_ms = cpu time to miliseconds
	printf("-----Time quanta used = %d\n", (p->p_quantum_size_ms) - (cpu_time_2_ms(p->p_cpu_time_left)));
	...
	if (quantum != -1) {
		p->p_quantum_size_ms = quantum; //updated quantum
		p->p_cpu_time_left = ms_2_cpu_time(quantum); //miliseconds to cpu time
	}
#ifdef CONFIG_SMP
	if (cpu != -1)
		p->p_cpu = cpu;
